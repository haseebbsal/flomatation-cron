...
cron.schedule('* * * * *', async () => {
    ...
    users.forEach(async (e) => {
        ...
        const checkAccessToken = await fetch(`https://services.leadconnectorhq.com/opportunities/search?limit=1&startAfter=0&location_id=${locationId}&pipeline_stage_id=4cf268bc-44f3-4098-b087-61cfcffcf314&status=open`, { headers: { 'Authorization': `Bearer ${accessToken}`, 'Version': '2021-07-28' } })
        ...
        if (!data.opportunities) {
            ...
            async function fetchRecursiveFlomatation() {
                let startAfter = 0
                while (true) {
                    const checkAccessToken = await fetch(`https://services.leadconnectorhq.com/opportunities/search?limit=100&startAfter=${startAfter}&location_id=${locationId}&pipeline_stage_id=4cf268bc-44f3-4098-b087-61cfcffcf314&status=open`, { headers: { 'Authorization': `Bearer ${accessToken}`, 'Version': '2021-07-28' } })
                    const data = await checkAccessToken.json()
            ...
            if (data.opportunities.length != 0) {
                ...
                for (let j of allDataEncircle) {
                    if (j.status == 'open') {
                        const uploadToEncircle = await fetch('https://api.encircleapp.com/v1/property_claims', { method: "POST", body: JSON.stringify({ organization_id: organisationId, brand_id, policyholder_name: j.name, policyholder_email_address: j.email, policyholder_phone_number: j.phone, project_manager_name: j.ownerName, full_address:j.address,assignment_identifier:j.name }),headers: { "Authorization": "Bearer 05e8a67f-1984-4107-95f9-c464ca6ca19e","Content-Type":"application/json" } })
                        console.log(await uploadToEncircle.json())
                    }
                }
                console.log('Uploaded Successfully')
            }
